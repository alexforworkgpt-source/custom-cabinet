...
Дата фиксации: 9 сентября 2026 года<br>
Исходная точка: `e0b623e1cbe83b1697c24325ccb98c27ba6cdd46` (`origin/main`) и реализация Issue 02<br>
Версия проекта: `1.69.1`
...
|---|---|---:|---:|---:|
| Главная | `/` | Да | Да | Да |
| Подписки | `/subscriptions` | Нет, compatibility URL | Нет | Да, как контекст Главной |
...
| Инструкции и настройка | `/instructions` | Через Profile и Support | Нет | Через Profile и Support |
...
```text
Главная | Тарифы | Поддержка | Профиль
```

Unread tickets показываются badge на пункте Поддержка. Feature-функции не
занимают постоянные слоты. Панель показывается только на маршрутах своих кнопок,
а также на `/subscriptions` и `/subscriptions/:subscriptionId`, потому что эти
маршруты остаются контекстом единого Dashboard и карточки подписки. На продлении,
пополнении, других вложенных страницах и в админке панель не монтируется и место
под неё не резервируется.
...
Быстрый вход в QR доступен на Dashboard рядом с кнопкой копирования ссылки
подписки. Обе кнопки используют одну и ту же разрешённую для отображения ссылку;
при скрытой или отсутствующей ссылке быстрый QR не показывается. Кнопка QR в
настройках VPN сохранена. Возврат с QR ведёт к выбранной подписке на Dashboard
(`/?sub=:id`) либо к исходному шагу `/connection`, в зависимости от точки входа.
...
Заголовок «Поддержка» и кнопка «Новый тикет» сохраняют компоновку из `7c88d7b`:
начиная с `sm` они расположены в одной строке, кнопка справа; на узких экранах
кнопка находится под заголовком и занимает всю ширину.

Под кнопкой «Новый тикет», перед списком обращений, показан блок «Инструкции и
настройка» со ссылкой на `/instructions`. В режиме внешней/Telegram-поддержки
без тикетов он остаётся перед кнопкой связи. В открытой форме
и переписке блок скрыт, чтобы не предлагать уход от несохранённого сообщения.
Переход сохраняет `instructionsOrigin: 'support'` в `location.state`: каталог,
статьи и связанные материалы показывают «Вернуться в поддержку» и сохраняют
этот контекст при переходах между инструкциями. Чтение не является обязательным
условием обращения; новые запросы к API и проверки прочтения не добавлены.
...
| Маршрут | Зависимость |
|---|---|
| `/connection/qr` | Ожидает `location.state` от Dashboard или `/connection`; сохраняет путь возврата |
