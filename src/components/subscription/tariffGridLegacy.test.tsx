...
describe('карточка тарифа для старой подписки', () => {
  it('предлагает перейти на тариф, а не выбрать тариф для продления', async () => {
  ...
  it('не считает отсутствие tariff_id достаточным без явного флага Bot', async () => {
    const { render, cardFor } = await import('./tariffGridHarness');
    render([{ id: 1, name: 'Базовый' }], {
      subscription: { id: 42, is_active: true, is_trial: false },
    });

    const card = cardFor('Базовый');
    expect(card.textContent).not.toContain('subscription.cta.moveToTariff');
    expect(card.textContent).toContain('subscription.purchase');
  });
