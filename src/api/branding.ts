...
const fetchLogo = async (logoUrl: string): Promise<Response> => {
  try {
    return await fetch(logoUrl);
  } catch {
    return fetch(logoUrl, { cache: 'reload' });
  }
};

const loadLogoBlob = async (logoPath: string): Promise<void> => {
  try {
...
export const preloadLogo = async (branding: BrandingInfo): Promise<void> => {
  if (!branding.has_custom_logo || !branding.logo_url || _logoBlobUrl) {
    return;
  }
