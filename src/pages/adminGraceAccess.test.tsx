...
vi.mock('@/store/permissions', () => ({
  usePermissionStore: (selector: (state: unknown) => unknown) =>
    selector({
      hasPermission: (permission: string) => permission !== 'settings:edit' || permissions.canEdit,
    }),
}));
...
it('keeps grace settings read-only without settings:edit', async () => {
  permissions.canEdit = false;
  await renderPage();
  expect((screen.getByLabelText('Длительность, часов') as HTMLInputElement).disabled).toBe(true);
  expect((modeCard('Включён') as HTMLButtonElement).disabled).toBe(true);
...
describe('changedFields', () => {
  ...
  it('пропускает недобранное число, но не пустой сквад', async () => {
    const { changedFields } = await import('@/components/admin/grace-access/form');
