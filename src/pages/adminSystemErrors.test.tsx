    ...
    adminSystemErrorsApi: {
      ...
      getAll: () =>
        listState.failed
          ...
          : Promise.resolve({ items: [ITEM], total: 1, limit: 50, offset: 0 }),
...
it('shows a retryable load error instead of an empty journal', async () => {
  listState.failed = true;
  renderPage();
  expect(await screen.findByRole('alert')).toBeTruthy();
  expect(screen.queryByText(resolveRu('admin.systemErrors.empty') as string)).toBeNull();
  listState.failed = false;
  fireEvent.click(screen.getByRole('button', { name: resolveRu('common.retry') }));
  expect(await screen.findByText(ITEM.event)).toBeTruthy();
});
